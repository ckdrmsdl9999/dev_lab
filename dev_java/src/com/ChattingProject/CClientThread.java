		...
		try {
			...
			if(note !=null){
				...
				case Protocol.ROOM_OUT: {
					...
					String nickName = st.nextToken();
					String roomName = st.nextToken();
					System.out.println("클라이언트스레드 룸네임:"+roomName);
					//방에서 나갔을때 이름 지우기
					for (int i = 0; i < ccl.cc.cr.jt_userList.getRowCount(); i++) {
						String delName = (String) ccl.cc.cr.dtm_userList.getValueAt(i, 0);
						if (delName.equals(nickName)) {
							ccl.cc.cr.dtm_userList.removeRow(i);
						}
					}
					//방에서 모두 나갔을때 방목록에서 방 지우기
					if (ccl.cc.cr.dtm_userList.getRowCount() == 0) {
						for (int i = 0; i < ccl.cc.cw.dtm_room.getRowCount(); i++) {
							String delRoom = (String) ccl.cc.cw.dtm_room.getValueAt(i, 0);
							if (delRoom.equals(roomName)){
				...
				case Protocol.CLOSE: {// 종료
					...
					for (int i = 0; i < ccl.cc.cw.jt_userList.getRowCount(); i++) {
						String delName = (String) ccl.cc.cw.dtm_userList.getValueAt(i, 0);
						if (delName.equals(nickName)) {
							ccl.cc.cw.dtm_userList.removeRow(i);
						}
					}
