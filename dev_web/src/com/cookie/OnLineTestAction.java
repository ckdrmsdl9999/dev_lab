...
public class OnLineTestAction implements Action {
	 ...
	 * -> OnLineTestLogic클래스의 marking메소드 호출
	 * -> OnLineTestDao클래스의 takeInsert메서드 호출 - INSERT 
	 * -> (1)OnLineTestDao클래스의 getRightAnswer메소드 호출 - SELECT
	 * -> (2)OnLineTestDao클래스의 getTake메소드 호출 - SELECT
	 * -> 위 1번과 2번 값 비교 - 채점하고 평가 점수 산출
	 * -> OnLineTestDao클래스의 examineeUpdate메소드 호출 - UPDATE
	 * -> OnLineTestDao클래스의 getExamineeDetail 메소드 호출 - SELECT 
	 * 
	 **************************************************************************/
	@Override           
...
/*			else if("test5".equals(cTest[i].getName())){
		...
		logger.info("5번 답안:"+test5);
		//다음으로 할 일은 사용자가 입력한 답안지 정보를 TakeVO에 담아야 합니다.
		//작성한 답안지 정보 출력해 보기 시작
		logger.info("take1:"+ptVO.getTake1());//1
		logger.info("take2:"+ptVO.getTake2());//1
		logger.info("take3:"+ptVO.getTake3());//2
		logger.info("take4:"+ptVO.getTake4());//2
		logger.info("take5:"+ptVO.getTake5());//2
		//현재 인증과정이 처리가 안되어 있어 수험번호가 담겨(쿠키에)있지 않습니다.
		//작성한 답안지 정보 출력해 보기 끝
		ActionForward forward = new ActionForward();
		OnLineTestLogic testLogic = new OnLineTestLogic();
		/*
		 * @param peVO(쿠키에 담겨있는 이름)
		 * @param ptVO(쿠키에 담겨있는 수험번호,문제코드,답안지)
		 */
		...
		Cookie c3 = new Cookie("cname",URLEncoder.encode(reVO.getT_name(), "utf-8"));
