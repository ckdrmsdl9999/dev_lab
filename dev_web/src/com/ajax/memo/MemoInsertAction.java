...
/*
 * easyui-textbox 속성을 사용할 때 주의사항
 * data-options속성을 사용하면 서버쪽에 값이 전달되지 않음.		
 */
		logger.info("to_id:"+req.getParameter("to_id"));
		logger.info("from_id:"+req.getParameter("from_id"));
		logger.info("memo_content:"+req.getParameter("memo_content"));//null
		logger.info("password:"+req.getParameter("password"));//null
		...
		else forward.setPath("./memoInsertFail.jsp");
		return forward;
	}
