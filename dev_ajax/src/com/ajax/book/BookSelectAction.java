...
public class BookSelectAction implements Action {
	...
	throws ServletException, IOException {
		...
		pMap.put("ab_no", req.getParameter("ab_no"));
		List<BookVO> bookList = bkLogic.getBookList(pMap,req);
		logger.info(req.getParameter("ab_no"));
		...
		if(pMap.get("choMode")!=null){
			forward.setPath("./searchBookResult.jsp"); //./jSonBookList.jsp
		}
		else if(pMap.get("ab_no")!=null){
			logger.info(pMap.get("ab_no"));
